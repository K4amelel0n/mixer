...
//        model.processStopped()
    ...
    private func getIcon(file path: String) -> NSImage? {
    ...
    func getIcon() -> NSImage?{
        guard let path = NSWorkspace.shared.absolutePathForApplication(withBundleIdentifier: self.bundleID)
        else{return nil}
        return self.getIcon(file:path)
    }
}
